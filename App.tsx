...
const App: React.FC = () => {
  ...
  const handleVerifySpot = async (capturedBase64: string) => {
    ...
    // Se a IA confirmar OU houver erro mas quisermos ser extremamente permissivos em testes:
    // Para resolver o "erro de conexão", garantimos que se o resultado for nulo, avisamos.
    if (result.isCorrectSpot || result.confidence >= 20) {
      ...
      setIsVerifying(false);
      showMsg(result.feedback || "Tente novamente.", 'error');
    }
  };
  ...
  return (
    <div className="flex flex-col flex-1 w-full h-full bg-slate-950 overflow-x-hidden">
        ...
        <div className="flex flex-col flex-1 animate-in fade-in duration-500">
          ...
          <div className="px-6 space-y-8">
            <button 
              ...
              className="w-full flex flex-col items-center justify-center p-10 bg-slate-900 rounded-[40px] border border-slate-800 shadow-xl active:scale-95 transition-all group"
            >
              ...
              {locations.map(loc => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="w-14 h-14 rounded-2xl overflow-hidden bg-slate-800 border-2 border-slate-700 shadow-lg relative shrink-0">
                      {loc.referenceImage ? (
                    ...
                    <div className="min-w-0">
                      <h3 className="font-bold text-white group-hover:text-indigo-400 transition-colors truncate">{loc.name}</h3>
                  ...
                  <button 
                    ...
                    className="p-3 text-slate-700 hover:text-red-500 shrink-0"
                  >
      ...
      {mode === AppMode.PATROL && activeLocation && (
        <div className="flex flex-col flex-1 animate-in slide-in-from-right duration-500 overflow-hidden">
          <header className="mt-8 mb-6 flex justify-between items-center px-6">
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="text-white font-bold truncate max-w-[150px]">{activeLocation.name}</span>
            </div>
          ...
          <main className="flex-1 flex flex-col justify-center px-6 pb-10">
            {(!currentCoords || (navState?.distance || 100) >= 6.5) ? (
              ...
              <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                 <div className="text-center bg-indigo-600/10 p-5 rounded-3xl border border-indigo-500/30">
      ...
      {mode === AppMode.REGISTRY && (
        <div className="flex flex-col flex-1 animate-in slide-in-from-bottom duration-500 px-6">
          ...
          {!referenceImage ? (
            <div className="flex-1 flex flex-col">
              <h2 className="text-2xl font-black text-white mb-2 tracking-tight text-center">Captura de Ponto</h2>
              <p className="text-slate-500 text-sm mb-6 text-center">Fotografe o objeto ou local da ronda.</p>
            ...
            <div className="flex-1 flex flex-col animate-in fade-in duration-300">
              ...
              <button 
                ...
                className="mb-10 w-full bg-indigo-600 py-6 rounded-[30px] font-black text-lg shadow-xl shadow-indigo-500/30 flex items-center justify-center space-x-3 active:scale-95 transition-all"
              >
      ...
      {message && (
        <div className={`fixed bottom-10 inset-x-4 p-6 rounded-[35px] border-2 shadow-2xl backdrop-blur-md flex items-center space-x-4 z-50 animate-in fade-in slide-in-from-bottom-10 ${message.type === 'success' ? 'bg-green-500/95 border-green-400 shadow-green-500/40' : 'bg-red-500/95 border-red-400'}`}>
