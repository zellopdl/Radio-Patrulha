...
import { 
  Plus, Navigation, Trash2, ChevronLeft, 
...
const App: React.FC = () => {
  ...
  const [matchInfo, setMatchInfo] = useState<{ type: string, feedback: string } | null>(null);
  ...
  useEffect(() => {
    const isNear = navState?.distance !== undefined && navState.distance < 12.0;
  ...
  const handleAutoVerify = async (frame: string, reference: string) => {
    ...
    if (result.isCorrectSpot) {
      ...
      setMatchInfo({ 
        ...
        feedback: result.feedback || "Parece que você encontrou!" 
      });
      setSuccessState(true);
      showMsg("LOCALIZADO!", 'success');
    } else {
  ...
  const handleSaveLocation = () => {
    if (!currentCoords || !newName.trim() || !referenceImage) {
      showMsg("Preencha o nome e capture a foto!", "error");
      return;
    }
    db.saveLocation({
      name: newName,
    ...
    showMsg("LOCAL SALVO COM SUCESSO!", 'success');
  };
      ...
      {mode === AppMode.DASHBOARD && (
        <div className="flex flex-col flex-1 animate-in fade-in w-full">
          <header className="mt-12 mb-8 flex flex-col items-center px-6">
            ...
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-1">Busca Visual Inteligente</p>
          </header>

          <div className="px-6 space-y-6 flex-1">
            <button 
              onClick={() => setMode(AppMode.REGISTRY)} 
              className="w-full flex flex-col items-center justify-center p-10 bg-white rounded-[40px] border border-slate-200 shadow-sm active:scale-95 transition-all"
            >
              <div className="w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center mb-3">
                <Plus className="w-6 h-6 text-indigo-600" />
              </div>
              <span className="font-bold text-xs uppercase tracking-widest text-slate-500">Cadastrar Novo Objeto</span>
            </button>

            <div className="space-y-4 pb-20">
              <h2 className="text-slate-400 font-bold text-[10px] uppercase tracking-widest px-2">Objetos a Encontrar</h2>
              {locations.length === 0 && (
                <div className="text-center py-12 bg-white/40 rounded-[40px] border border-dashed border-slate-200 text-slate-400 text-sm">
                  Lista de alvos vazia.
                </div>
              )}
              {locations.map(loc => (
                <div key={loc.id} onClick={() => { setActiveLocation(loc); setMode(AppMode.PATROL); }} className="bg-white p-4 rounded-[35px] border border-slate-200 flex items-center justify-between active:bg-slate-50 shadow-sm transition-all group">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); if(confirm('Remover este alvo?')){ db.deleteLocation(loc.id); setLocations(db.getLocations()); }}} className="p-3 text-slate-300 hover:text-red-500 transition-colors">
      ...
      {mode === AppMode.PATROL && activeLocation && (
        <div className="flex flex-col flex-1 animate-in slide-in-from-right w-full h-full">
          <header className="mt-8 mb-6 flex justify-between items-center px-6">
            <button onClick={resetToHome} className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
              <ChevronLeft className="w-6 h-6 text-slate-900" />
            </button>
            <div className="flex flex-col items-center">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Alvo Atual</span>
            ...
            {successState ? (
              <div className="flex flex-col items-center justify-center space-y-10 animate-in zoom-in duration-500">
                <div className="relative">
                  <div className="absolute inset-0 bg-emerald-400/30 blur-3xl rounded-full scale-150 animate-pulse" />
                  <div className="w-32 h-32 bg-emerald-500 rounded-full flex items-center justify-center shadow-2xl relative z-10 animate-bounce">
                    <CheckCircle2 className="w-16 h-16 text-white" />
                  </div>
                </div>
                
                <div className="text-center space-y-3 px-6">
                  <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tight">PARABÉNS!</h2>
                  <p className="text-slate-600 font-bold text-lg">Você encontrou o local!</p>
                  
                  <div className="bg-emerald-50 p-4 rounded-3xl border border-emerald-100 inline-block mt-4">
                    <p className="text-emerald-700 font-bold text-sm italic">"{matchInfo?.feedback}"</p>
                  </div>
                  
                  {matchInfo?.type === 'SIMILAR' && (
                    <p className="text-amber-600 text-[10px] font-black uppercase tracking-widest mt-2 flex items-center justify-center">
                      <Sparkles className="w-3 h-3 mr-1" /> Reconhecido por similaridade de categoria
                    </p>
                  )}
                </div>
                ...
                <button 
                  onClick={resetToHome}
                  className="w-full bg-slate-900 text-white py-6 rounded-[35px] font-black text-lg shadow-2xl flex items-center justify-center space-x-3 active:scale-95 transition-all"
                >
              ...
              <div className="space-y-6 animate-in fade-in flex flex-col flex-1">
                 <div className="text-center bg-indigo-50 p-6 rounded-[40px] border border-indigo-100 shadow-sm shrink-0">
                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest animate-pulse">Procurando Alvo...</p>
                    <p className="text-slate-900 text-base font-black mt-1">Aponte a câmera para o objeto</p>
                    <p className="text-slate-500 text-xs">O sistema irá reconhecer automaticamente.</p>
                 </div>
                 <ARView 
                    ref={arRef} 
      ...
      {mode === AppMode.REGISTRY && (
        <div className="flex flex-col flex-1 animate-in slide-in-from-bottom w-full px-6">
          <header className="mt-8 mb-8 flex items-center justify-between">
            ...
            <span className="text-slate-400 font-black text-[10px] uppercase tracking-widest">Registrar Alvo</span>
          ...
          {!referenceImage ? (
            ...
            <div className="flex-1 flex flex-col animate-in fade-in pb-10">
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">Nome do Objeto/Local</label>
                <input 
                  type="text" value={newName} onChange={(e) => setNewName(e.target.value)}
                  placeholder="Ex: Minha Cafeteira"
                  className="w-full bg-white border border-slate-200 p-6 rounded-[30px] text-lg font-bold text-slate-900 outline-none shadow-sm focus:border-indigo-500"
                />
                <button onClick={handleSaveLocation} className="w-full bg-indigo-600 py-7 rounded-[35px] font-black text-lg shadow-xl text-white flex items-center justify-center space-x-3 active:scale-95 transition-all">
